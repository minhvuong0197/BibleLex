...
export async function GET() {
  ...
  const items = await prisma.highlight.findMany({ where: { userId: u.sub }, orderBy: { createdAt: "asc" } })
...
export async function POST(req: NextRequest) {
  ...
  const color = typeof b.color === "string" && b.color ? b.color : "yellow"
  ...
  const item = await prisma.highlight.upsert({
    ...
    update: { color },
    create: { userId: u.sub, book, chapter, verse, color },
  })
...
export async function DELETE(req: NextRequest) {
  ...
  await prisma.highlight.deleteMany({ where: { userId: u.sub, book, chapter, verse } })
